...
final class MacroPadProtocolTests: XCTestCase {
  ...
  func testExtendedMediaReportUsesReportIDTwoMapping() throws {
    ...
    let report = try XCTUnwrap(
      MacroPadProtocolEncoder.reports(
  ...
  func testLightingIsLayerScoped() throws {
    ...
    let report = try XCTUnwrap(
      MacroPadProtocolEncoder.lightingReport(
  ...
  func testSequenceLengthIsValidated() {
    ...
    XCTAssertThrowsError(
      try MacroPadProtocolEncoder.reports(
        for: .key(1), layer: 0, binding: binding, profile: extendedProfile, reportID: 0
      )
    ) { error in
      XCTAssertEqual(
        error as? ProtocolEncodingError,
        .sequenceTooLong(maximum: extendedProfile.maxSequenceLength))
    }
  }
  ...
  func testBatteryLevelAcceptsOnlyPercentages() {
    XCTAssertEqual(ConnectedHIDDevice.normalizedBatteryLevel(0), 0)
    XCTAssertEqual(ConnectedHIDDevice.normalizedBatteryLevel(100), 100)
    XCTAssertNil(ConnectedHIDDevice.normalizedBatteryLevel(-1))
    XCTAssertNil(ConnectedHIDDevice.normalizedBatteryLevel(101))
  }
  ...
  func testConfigurationDecoderRejectsIncompleteResponse() {
    XCTAssertThrowsError(
      try MacroPadProtocolDecoder.applying(
