...
enum HIDServiceError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .reportWriteFailed(let result): "A configuration report could not be written to the macro pad (IOKit \(result))."
        case .reportReadFailed(let result): "A configuration report could not be read from the macro pad (IOKit \(result))."
        case .readTimedOut(let expected, let actual): "The macro pad returned \(actual) of \(expected) configuration records before timing out."
    ...
    private func read(service: io_service_t, requests: [HIDReport]) throws -> [HIDInputReport] {
        ...
        IOHIDDeviceRegisterInputReportCallback(
            ...
            { context, result, _, _, reportID, report, reportLength in
                ...
                let normalized = bytes.first == UInt8(clamping: reportID)
                    ? Array(bytes.dropFirst())
                    : bytes
                reader.reports.append(HIDInputReport(reportID: UInt8(clamping: reportID), payload: normalized))
            },
            context
        )
        IOHIDDeviceScheduleWithRunLoop(device, runLoop, runLoopMode.rawValue)
