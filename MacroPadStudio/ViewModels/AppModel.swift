...
final class AppModel: ObservableObject {
  ...
  var connectionDetail: String {
    ...
    case .hardware(let device):
      if device.isConfigurable {
        String(
          format: "%@ • %04X:%04X", device.transport, device.profile.vendorID,
          device.profile.productID)
      } else {
  ...
  var batteryLevel: Int? {
    guard case .hardware(let device) = mode else { return nil }
  ...
  func moveKeystrokeUp(id: UUID) {
    updateSelectedBinding { binding in
      guard let index = binding.sequence.firstIndex(where: { $0.id == id }), index > 0 else {
        return
      }
  ...
  func moveKeystrokeDown(id: UUID) {
    updateSelectedBinding { binding in
      guard let index = binding.sequence.firstIndex(where: { $0.id == id }),
        index + 1 < binding.sequence.count
      else { return }
      binding.sequence.swapAt(index, index + 1)
    }
  }
  ...
  private func writePendingChanges(source: WriteSource) {
    ...
    do {
      for location in pendingControls.sorted(by: { $0.id < $1.id }) {
        ...
        guard let binding = configuration.layers[location.layer].bindings[location.control] else {
          continue
        }
      ...
      notice = Notice(
        kind: .success, message: successMessage(for: source, deviceName: connected.productName))
      persistDraft()
    } catch {
  ...
  func readFromDevice() {
    if case .demo = mode {
      ...
      guard canReadFromDevice else {
        notice = Notice(
          kind: .warning, message: "Configuration reading is not supported for this connection.")
        return
      }
  ...
  private func loadConfigurationFromDevice(from connected: ConnectedHIDDevice, automatic: Bool) {
    ...
    Task {
      ...
      do {
        ...
        persistDraft()
        let assignmentCount =
          connected.profile.layerCount
          * (connected.profile.keyCount + connected.profile.knobCount * 3)
        notice = Notice(
          kind: .success,
          message:
            "Read \(assignmentCount) assignments from \(connected.productName). Layer lighting was preserved."
        )
      } catch {
  ...
  private func syncPendingChangesIfNeeded() {
    ...
    guard canWriteToDevice else {
      if isHardwareConnected {
        notice = Notice(
          kind: .warning, message: "Changes are local only. Connect USB to store them.")
      }
      return
    }
  ...
  private func persistDraft() {
    do {
      ...
      notice = Notice(
        kind: .error, message: "Could not save the local draft: \(error.localizedDescription)")
    }
  }
}
